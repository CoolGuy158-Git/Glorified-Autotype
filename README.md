...
### Context pairs
Context pairs provides bias.

say main pair gets cats+are ["small", "big", "strong", "puppies", cute"]

and context pair is small+cats ["are", "cute"]

It increases cute's score so now when random picks a random word that picks from the selection the bias makes it tilt towards generating "cute".
